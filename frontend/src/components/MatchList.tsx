...
export default function MatchList({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_IP}/matches/${currentUserId}`, {
      signal,
    })
      .then((res) => {
        ...
        <ul>
          {matches.map((match) => {
            ...
            const userName = isWin
              ...
              : match.looser.username; // Ton pseudo
            const opponentName = isWin
              ? match.looser.username
              : match.winner.username; // Pseudo de l'adversaire
