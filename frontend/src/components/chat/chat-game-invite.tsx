...
const GameInviteMessage: React.FC<GameInviteMessageProps> = ({
  ...
  const isCreatorWinner: boolean = gameInvite.creatorScore > gameInvite.recipientScore;
    ...
    <Card className="w-full p-3 py-1 rounded-lg border-none bg-gray-900 shadow-none flex flex-col justify-start">
      <div className="text-center p-2">
        ...
        <div className="relative flex justify-center mb-2">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 bg-gray-700 text-gray-300 px-2 py-0.5 rounded text-xs z-10">VS</div>
          
          {/* Player names with fixed spacing */}
          <div className="flex justify-center w-full items-center">
            <div className={`flex items-center justify-end flex-1 max-w-[45%] pr-5 ${
              isGameCompleted 
                ? (isCreatorWinner ? "text-green-500" : "text-red-500")
                : "text-blue-400"
            }`}>
              {isCreator ? "You" : gameInvite.creatorUsername}
              {isGameCompleted && (isCreatorWinner 
                ? <Crown className="w-4 h-4 ml-1 text-yellow-400" />
                : <Skull className="w-4 h-4 ml-1" />
              )}
            </div>
            <div className="invisible px-2">VS</div> {/* Invisible placeholder */}
            <div className={`flex items-center justify-start flex-1 max-w-[45%] pl-5 ${
              isGameCompleted 
                ? (!isCreatorWinner ? "text-green-500" : "text-red-500")
                : "text-blue-400"
            }`}>
        ...
        {isGameCompleted && (
          <div>
            <div className="flex justify-center items-center gap-2 mb-1">
              <span className={isCreatorWinner ? "text-green-500" : "text-red-500"}>
